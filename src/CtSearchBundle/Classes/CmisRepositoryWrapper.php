...
class CmisRepositoryWrapper
{
    ...
    static function extractObjectFromNode($xmlnode)
    {
        ...
        foreach ($prop_nodes as $pn) {
            if ($pn->attributes) {
                if (!empty($pn->attributes->getNamedItem("propertyDefinitionId")->nodeValue)) {
                    $key = $pn->attributes->getNamedItem("propertyDefinitionId")->nodeValue;

                    $items = $pn->getElementsByTagName("value");
                    ...
                    $retval->properties[$key] = $data;

                    if (count($data) > 1) {
                        $retval->properties[$key] = $data;
                    } else {
                        if (!empty($pn->getElementsByTagName("value")->item(0)->nodeValue)) {
                            $retval->properties[$key] = $pn->getElementsByTagName("value")->item(0)->nodeValue;
                        }
                    }
                }
            }
