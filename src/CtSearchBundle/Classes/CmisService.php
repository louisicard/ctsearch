...
# under the License.
use \CtSearchBundle\Classes\CmisRepositoryWrapper;
...
/* Utility functions */
	...
	function getPropertyType($typeId, $propertyId) {
		if (!empty($this->_type_cache[$typeId]->properties[$propertyId]["cmis:propertyType"])) {
	...
	function getObjectType($objectId) {
		if (isset($this->_objTypeId_cache[$objectId])) {
	...
	function getTitle($objectId) {
		if (!empty($this->_title_cache[$objectId])) {
